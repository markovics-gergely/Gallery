...
export class FavoritesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      if (params['id']) {
        ...
        this.albumService.getFavoriteAlbums(environment.default_page_size, environment.default_page).subscribe(albums => {
  ...
  setPage(value: PagerModel) {
    this.albumService.getFavoriteAlbums(value.pageSize, value.page + 1).subscribe(albums => {
      this._galleries = albums.values;
      this._total = albums.total;
    });
  }
